...
import { linkClicks, matches, offerProducts, offers } from "@/db/schema";
...
export async function GET(request: Request, context: { params: Promise<{ code: string }> }) {
  ...
  const matchRows = await db
    .select({ id: matches.id, offerId: matches.offerId, brandId: offers.brandId })
    .from(matches)
  ...
  if (!store) return Response.redirect(new URL("/", request.url), 302);
  ...
  let targetUrl = `https://${store.shopDomain}`;
