        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
              ...
              <div className="relative border-4 border-neon-pink bg-card overflow-hidden pixel-border-pink">
                ...
                <div className="p-4">
                  ...
                  <div className="mt-4 flex items-center justify-center gap-4">
                    <button className="w-12 h-12 border-4 border-destructive bg-destructive/10 flex items-center justify-center pixel-btn" type="button">
                      <span className="font-pixel text-destructive text-lg">X</span>
                    </button>
                    <button className="w-14 h-14 bg-neon-green border-4 border-neon-green flex items-center justify-center pixel-btn glow-green" type="button">
          ...
          <div className="order-1 lg:order-2">
            ...
            <div className="grid sm:grid-cols-2 gap-4 mb-10">
              {perks.map((perk) => {
                const c = colorMap[perk.color as keyof typeof colorMap];
                return (
                  <div
                    key={perk.title}
                    className={`flex gap-3 p-4 border-2 border-border ${c.hoverBorder} transition-all group`}
                  >
                    <div className={`w-10 h-10 border-2 ${c.border} ${c.bgSoft} flex items-center justify-center flex-shrink-0`}>
                      <perk.icon className={`w-5 h-5 ${c.text}`} />
                    </div>
                    <div>
                      <h3 className={`font-pixel text-xs mb-1 ${c.text}`}>{perk.title}</h3>
