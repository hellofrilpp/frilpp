    ...
    <section id="pricing" className="py-24 bg-card border-t-4 border-border relative">
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center max-w-2xl mx-auto mb-16">
          ...
          <p className="font-mono text-sm text-muted-foreground">&gt; Simple monthly pricing. Cancel anytime.</p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3 max-w-5xl mx-auto">
          {plans.map((plan) => {
            ...
            return (
              <div
                key={plan.name}
                className={`relative p-6 border-4 ${plan.popular ? `${c.border} ${c.bgSoft}` : "border-border bg-background"} transition-all hover:translate-x-1 hover:translate-y-1`}
              >
                {plan.popular && (
                  <div className="absolute -top-4 left-1/2 -translate-x-1/2">
                    <div className={`px-4 py-1 ${c.bgSolid} text-background text-xs font-pixel`}>POPULAR</div>
                  </div>
                )}
                ...
                <div className="text-center mb-6 pt-2">
                  ...
                  <div className="flex items-baseline justify-center gap-1 mb-2">
                    ...
                    {plan.period && <span className="text-xs font-mono text-muted-foreground">{plan.period}</span>}
                  </div>
                ...
                {isMailto ? (
                  <a
                    href={plan.href}
                    className={`w-full inline-flex items-center justify-center py-5 text-xs font-pixel ${plan.popular ? `${c.bgSolid} text-background ${c.glow}` : "bg-foreground text-background hover:bg-foreground/90"} pixel-btn`}
                  >
                    {plan.cta}
