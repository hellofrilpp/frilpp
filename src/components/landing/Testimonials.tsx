      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {highlights.map((item) => (
            ...
            >
              <Quote className={`w-6 h-6 mb-4 ${item.type === "brand" ? "text-neon-green" : "text-neon-pink"}`} />

              <div className="flex gap-1 mb-4">
                {[...Array(5)].map((_, i) => (
                  <Star key={`${item.title}-${i}`} className="w-4 h-4 fill-neon-yellow text-neon-yellow" />
                ))}
              </div>

              <p className="font-pixel text-xs text-foreground mb-2">{item.title}</p>
              <p className="font-mono text-sm leading-relaxed text-foreground">{item.body}</p>
            </div>
          ))}
        </div>
      </div>
